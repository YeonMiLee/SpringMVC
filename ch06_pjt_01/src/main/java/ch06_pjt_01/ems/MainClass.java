...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		//IoC 컨테이너 생성.		
		//설정파일 분리 전(applicationContext.xml 만 있을때)
		GenericXmlApplicationContext ctx = 
				new GenericXmlApplicationContext("classpath:applicationContext.xml");
		 
		
		//설정파일 분리 후(appCtx1,2,3 생성)
		/*
		 * GenericXmlApplicationContext ctx = new
		 * GenericXmlApplicationContext("classpath:appCtx1.xml","classpath:appCtx2.xml",
		...
		//데이터베이스에 샘플 데이터 등록
		StudentRegisterService registerService = ctx.getBean("stRegisterService", StudentRegisterService.class);
		...
		//학생등록
		registerService = ctx.getBean("stRegisterService", StudentRegisterService.class);
